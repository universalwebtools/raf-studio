// RAF.studio — keep editorDraft identical to last published state, preserving active editor version
...
async function syncToPublished(stamp,{reload=false}={}){
  ...
  try{
    ...
    const next={...d,site:siteSnap.val()||{},homeContent:contentSnap.val()||{},homeMedia:mediaSnap.val()||{},visualStyles:stylesSnap.val()||{},builder};
    ...
    if(reload){
      ...
      if(!sessionStorage.getItem(key)){
        ...
        u.searchParams.set('ev',new URLSearchParams(location.search).get('ev')||'7.0');
...
if(publishedAt&&baseAt!==publishedAt)await syncToPublished(publishedAt,{reload:true});
lastSeen=publishedAt;
onValue(ref(db,`${root}/publishedAt`),async snap=>{const stamp=Number(snap.val()||0);if(!stamp){lastSeen=stamp;return}if(lastSeen===null){lastSeen=stamp;return}if(stamp!==lastSeen){lastSeen=stamp;await syncToPublished(stamp,{reload:true})}});
