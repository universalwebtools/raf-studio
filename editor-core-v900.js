...
let renderingClones=false,migrationDirty=false;
...
function css(){
 ...
 'body.raf-e3 [data-v760-inline-edit="1"]{cursor:text!important;user-select:text!important;-webkit-user-select:text!important;caret-color:#22a8ff;outline:2px solid #fff!important;outline-offset:4px}'+
 ...
 '#v72box{position:fixed;z-index:1000013;border:2px solid #22a8ff;box-sizing:border-box;pointer-events:none}'+
 ...
 '#v72box.multi #v72move{background:#9835d5}#v72box.v900topSafe #v72move{top:4px}#v72box.v900topSafe #v72flow{top:4px;left:120px}#v72box.v900topSafe #v760scaleBadge{top:4px}'+
 '#v72box.multi .v760handle{border-color:#ad45eb;background:#fff;box-shadow:0 0 0 3px #ad45eb30}'+
 '#v760scaleBadge{position:absolute;right:5px;top:-31px;display:none;padding:5px 8px;border-radius:7px;background:#9835d5;color:#fff;font:800 10px system-ui;white-space:nowrap;pointer-events:none}'+
...
function id(el){
 ...
 if(cloneRoot&&cloneRoot!==el){const k=clonePart(el,cloneRoot);el.dataset.rafV72Id=k;return k}
 const fp=fingerprint(el),had=fp?stableIds[fp]:null,k=resolveObjectId(el,stableIds,true);
 if(!had&&k?.startsWith('node:'))stableDirty=true;
...
function cropApply(el,c,layoutScale=1){
 ...
 el.style.scale=String(Math.max(.05,(Number(c.zoom)||1)*ls));
...
function boxEnsure(){
 ...
 overlay.innerHTML='<div id="v72move">✥ PRZESUŃ</div><div id="v72flow"><button id="v72flowUp" title="Zamień miejscami z poprzednim elementem">↑ WYŻEJ</button><button id="v72flowDown" title="Zamień miejscami z następnym elementem">↓ NIŻEJ</button><button id="v72flowEdit" title="Otwórz edycję zaznaczenia">✎ EDYTUJ</button><button id="v72flowHide" title="Ukryj bez usuwania">◌ UKRYJ</button><button id="v72flowDelete" class="v760danger" title="Przenieś do kosza (można cofnąć)">⌫ USUŃ</button></div><div id="v760scaleBadge">100%</div><div id="v72rotate" title="Obrót"></div>'+['nw','n','ne','e','se','s','sw','w'].map(d=>'<div class="v760handle" data-dir="'+d+'" title="'+(d.length===2?'Skaluj cały zaznaczony zestaw':'Zmień rozmiar')+'"></div>').join('');
 ...
 $('#v72flowEdit').onclick=e=>{e.preventDefault();requestEdit()};
...
function boxUpdate(){
 ...
 const l=Math.max(4,b.left),t=Math.max(4,b.top),r=Math.min(innerWidth-4,b.right),bt=Math.min(innerHeight-4,b.bottom);
 Object.assign(o.style,{left:l+'px',top:t+'px',width:Math.max(18,r-l)+'px',height:Math.max(18,bt-t)+'px'});
 o.classList.toggle('v900topSafe',b.top<45);o.dataset.fullLeft=String(b.left);o.dataset.fullTop=String(b.top);o.dataset.fullWidth=String(b.width);o.dataset.fullHeight=String(b.height);
 $('#v72move').textContent=sel.size>1?'✥ PRZESUŃ '+sel.size+' ELEMENTY':'✥ PRZESUŃ';
 $$('.v760handle',o).forEach(x=>x.style.display=sel.size===1||(sel.size>1&&x.dataset.dir.length===2)?'block':'none');$('#v72rotate').style.display=sel.size===1?'block':'none';if(drag?.mode!=='multi-resize')$('#v760scaleBadge').style.display='none';
...
function commit(label='Układ'){window.rafHistory900?.begin?.(label)}
function save(label='Układ'){
 clearTimeout(saveTimer);const l=cp(layout),cl=cp(clones),f=cp(flowOrders),ids=cp(stableIds),writeIds=stableDirty;stableDirty=false;
 saveTimer=setTimeout(async()=>{try{const jobs=[set(ref(db,ROOT+'/freeLayoutV7'),l),set(ref(db,ROOT+'/clonesV76'),cl),set(ref(db,ROOT+'/flowOrderV772'),f)];if(writeIds)jobs.push(set(ref(db,STABLE_ROOT),ids));await Promise.all(jobs);window.rafHistory900?.commit?.(label);const s=$('#rafStatus3');if(s)s.textContent='✓ Wersja robocza zapisana'}catch(e){console.error(e);const s=$('#rafStatus3');if(s)s.textContent='⚠ Błąd zapisu: '+e.message}},90);
...
function multiResizeLive(e){
 ...
 d.items.forEach(i=>{const c=ownCfg(i.k,i.el),targetCx=d.anchorX+(i.cx-d.anchorX)*scale,targetCy=d.anchorY+(i.cy-d.anchorY)*scale;c.x=i.x;c.y=i.y;c.scale=Math.round(i.scale*scale*10000)/10000;apply(i.el);const r=i.el.getBoundingClientRect();c.x=i.x+targetCx-(r.left+r.width/2);c.y=i.y+targetCy-(r.top+r.height/2);apply(i.el)});
 const badge=$('#v760scaleBadge');if(badge){badge.textContent=Math.round(scale*100)+'%';badge.style.display='block'}
}
...
function flowParentKey(parent){
 ...
 const section=parent.closest('[data-raf-section],header.hero'),sectionKey=section?id(section):'page';if(parent===section)return'parent:'+sectionKey;
...
function flowChildren(parent){return[...parent.children].filter(x=>x.matches?.(CAND)&&!x.closest('#rafTop3,#rafPanel3,#rafProModal61,#tpl752,#widgetsModal770,#v72box,#v760layers,#v760menu,#v760history'))}
function flowState(){
 if(sel.size!==1)return null;const el=[...sel][0],parent=el.parentElement;if(!parent)return null;const nodes=flowChildren(parent),i=nodes.indexOf(el);if(i<0)return null;
 return{el,parent,nodes,index:i,canUp:i>0,canDown:i<nodes.length-1,key:flowParentKey(parent)}
}
function rememberFlow(parent,key){
 const items=flowChildren(parent).map(id);if(items.length<2)return;const old=flowOrders.find(x=>x.parent===key);if(old)old.items=items;else flowOrders.push({parent:key,items})
}
function applyFlowOrders(){
 if(!flowOrders.length)return;
 for(const row of flowOrders){
  const ordered=(row.items||[]).map(findById).filter(Boolean),parent=ordered[0]?.parentElement;if(!parent||flowParentKey(parent)!==row.parent)continue;
  const children=[...parent.children],positions=[];children.forEach((x,i)=>{if(ordered.includes(x)&&x.parentElement===parent)positions.push(i)});if(positions.length<2)continue;
  const desired=ordered.filter(x=>x.parentElement===parent),current=positions.map(i=>children[i]);if(current.every((x,i)=>x===desired[i]))continue;
  const final=[...children];positions.forEach((pos,i)=>{if(desired[i])final[pos]=desired[i]});final.forEach(x=>parent.appendChild(x))
 }
}
function flowStep(direction){
 const s=flowState();if(!s||(direction<0&&!s.canUp)||(direction>0&&!s.canDown))return;commit();
 const other=s.nodes[s.index+direction];if(direction<0)s.parent.insertBefore(s.el,other);else s.parent.insertBefore(other,s.el);
 rememberFlow(s.parent,s.key);save();boxUpdate();panel();s.el.scrollIntoView({behavior:'smooth',block:'center'});const status=$('#rafStatus3');if(status)status.textContent=direction<0?'✓ Element przesunięty o jeden poziom wyżej':'✓ Element przesunięty o jeden poziom niżej'
}
function panel(){
...
function resetTransform(){patchSelected({x:0,y:0,width:null,height:null,scale:1,rotate:0})}
...
function insertPlainText(value){
 ...
 selection.deleteFromDocument();const node=document.createTextNode(value),range=selection.getRangeAt(0);range.insertNode(node);range.setStartAfter(node);range.collapse(true);selection.removeAllRanges();selection.addRange(range)
}
...
function beginInlineEdit(el){
 ...
 if(inlineEdit?.el===el){el.focus({preventScroll:true});return true}
 if(inlineEdit)finishInlineEdit(false);if(!sel.has(el))select(el,false);disarmMove();commit();
 const k=id(el),original=String(cfg(k,el).text??inlineValue(el));
 const onInput=()=>{ownCfg(k,el).text=inlineValue(el);save()};
 const onBlur=()=>finishInlineEdit(false);
 const onKey=e=>{if(e.key==='Escape'){e.preventDefault();e.stopPropagation();finishInlineEdit(true);el.blur()}else if(e.key==='Enter'&&(e.ctrlKey||e.metaKey)){e.preventDefault();finishInlineEdit(false);el.blur()}};
 const onPaste=e=>{e.preventDefault();insertPlainText(e.clipboardData?.getData('text/plain')||'');onInput()};
 inlineEdit={el,k,original,onInput,onBlur,onKey,onPaste};el.dataset.v760InlineEdit='1';el.setAttribute('contenteditable','true');el.spellcheck=true;
 el.addEventListener('input',onInput);el.addEventListener('blur',onBlur);el.addEventListener('keydown',onKey);el.addEventListener('paste',onPaste);boxUpdate();
 requestAnimationFrame(()=>{el.focus({preventScroll:true});const selection=getSelection(),range=document.createRange();range.selectNodeContents(el);range.collapse(false);selection.removeAllRanges();selection.addRange(range)});
 const s=$('#rafStatus3');if(s)s.textContent='✎ Edycja na stronie — zaznaczaj, wpisuj i usuwaj tekst • Esc anuluje';window.dispatchEvent(new CustomEvent('raf:v760-inline-start',{detail:{id:k,element:el}}));return true
}
function requestEdit(){const el=sel.size===1?[...sel][0]:null;if(el&&beginInlineEdit(el))return;window.dispatchEvent(new CustomEvent('raf:v760-edit'))}
function parentCandidate(el){let p=el?.parentElement;while(p&&p!==document.body){if(p.matches?.(CAND)&&!p.closest('#rafTop3,#rafPanel3,#rafProModal61,#tpl752,#widgetsModal770,#v72box,#v760layers,#v760menu,#v760history'))return p;p=p.parentElement}return null}
function pendingMove(e,el){
 const items=[...sel].filter(x=>!cfg(id(x),x).locked).map(x=>({el:x,k:id(x),x:Number(cfg(id(x),x).x)||0,y:Number(cfg(id(x),x).y)||0}));if(!items.length)return;
...
window.addEventListener('pointermove',e=>{
 if(marq&&e.pointerId===marq.pid){const dx=e.clientX-marq.sx,dy=e.clientY-marq.sy;if(!marq.moved&&Math.hypot(dx,dy)>3){marq.moved=true;const m=document.createElement('div');m.className='v72marq';m.id='v72marq';document.body.appendChild(m)}if(marq.moved){const l=Math.min(marq.sx,e.clientX),t=Math.min(marq.sy,e.clientY),r=Math.max(marq.sx,e.clientX),bt=Math.max(marq.sy,e.clientY),m=$('#v72marq');Object.assign(m.style,{left:l+'px',top:t+'px',width:r-l+'px',height:bt-t+'px'});sel.forEach(x=>x.classList.remove('v72sel'));sel=new Set(marq.base);hits(l,t,r,bt).forEach(add);boxUpdate();e.preventDefault();e.stopPropagation();e.stopImmediatePropagation()}return}
 if(!drag||e.pointerId!==drag.pid)return;
 if(drag.mode==='pending'){if(Math.hypot(e.clientX-drag.sx,e.clientY-drag.sy)<3)return;commit('Przesunięcie');drag.mode='move'}
 if(drag.mode==='move'){let dx=e.clientX-drag.sx,dy=e.clientY-drag.sy;if(!(e.ctrlKey||e.metaKey)&&drag.startBox){const s=snapMove(drag.startBox,dx,dy,drag.targets||[]);dx+=s.dx;dy+=s.dy;guideShow(s)}else guideHide();drag.items.forEach(i=>{const cc=ownCfg(i.k,i.el);cc.x=i.x+dx;cc.y=i.y+dy;apply(i.el)});boxUpdate();e.preventDefault();e.stopPropagation();e.stopImmediatePropagation()}
 else if(drag.mode==='resize'){guideHide();resizeLive(e);boxUpdate();e.preventDefault();e.stopPropagation();e.stopImmediatePropagation()}
 else if(drag.mode==='multi-resize'){guideHide();multiResizeLive(e);boxUpdate();e.preventDefault();e.stopPropagation();e.stopImmediatePropagation()}
 else if(drag.mode==='rotate'){const cc=ownCfg(drag.k,drag.el),an=Math.atan2(e.clientY-drag.cy,e.clientX-drag.cx);cc.rotate=drag.start+(an-drag.angle)*180/Math.PI;if(e.shiftKey)cc.rotate=Math.round(cc.rotate/15)*15;apply(drag.el);boxUpdate();e.preventDefault();e.stopPropagation();e.stopImmediatePropagation()}
},true);
function finishPointer(e){
 if(marq&&e.pointerId===marq.pid){$('#v72marq')?.remove();marq=null;panel();boxUpdate();guideHide();emit();e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();return}
 if(drag&&e.pointerId===drag.pid){const mode=drag.mode,scaled=mode==='multi-resize',scale=drag.lastScale,count=drag.items?.length||0;drag.sourceEl?.classList.remove('v760moving');if(['move','resize','multi-resize','rotate'].includes(mode))save(mode==='move'?'Przesunięcie':mode==='rotate'?'Obrót':'Zmiana rozmiaru');drag=null;panel();boxUpdate();guideHide();emit();if(scaled){const s=$('#rafStatus3');if(s)s.textContent='✓ '+count+' elementów przeskalowano do '+Math.round(scale*100)+'%'}}
}
...
const observer=new MutationObserver(scheduleDecor);const observeRoots=()=>{observer.disconnect();for(const root of [document.querySelector('#rafTemplate752'),document.querySelector('#rafMain'),document.querySelector('.rafHeader900')].filter(Boolean))observer.observe(root,{subtree:true,childList:true});observer.observe(document.body,{childList:true})};observeRoots();window.addEventListener('raf:template752-rendered',observeRoots);
window.addEventListener('raf:history-main',e=>{const n=e.detail?.builder;if(n?.freeLayoutV7){layout=cp(n.freeLayoutV7);clones=cp(n.clonesV76||[]);flowOrders=cp(n.flowOrderV772||[]);applyAll();applyFlowOrders()}});
...
(async()=>{
 ...
 window.rafCore900={
  undo:undoNow,redo:redoNow,canUndo:()=>!!window.rafHistory900?.canUndo?.(),canRedo:()=>!!window.rafHistory900?.canRedo?.(),lastAt:()=>Date.now(),nextUndoAt:()=>0,nextRedoAt:()=>0,clearRedo:()=>{},applyLayout:x=>{layout=cp(x||{});applyAll()},
