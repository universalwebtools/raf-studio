// RAF.studio — editor/public parity + widget media bridge v8.8.6
...
if(EDITOR){
  ...
  function stampVisibleVersion(){
    ...
    const st=document.getElementById('rafStatus3');if(st&&/Edytor 8\.[0-9.]+/i.test(st.textContent||'')&&!/Zmiany|Opublik|Błąd|Cofanie|Ponawianie|Synchron|Usuw|Szablon/i.test(st.textContent||''))st.textContent='✓ Edytor '+VERSION+' • SELECTABLE HERO VIDEO + DIRECT CROP';
  }
  function reapplyPublicRuntime(){
    ...
    proTimer=setTimeout(()=>{
      try{
        ...
        window.rafHeroDirectCrop886?.refresh?.();
        stampVisibleVersion();
  ...
  window.rafParity886={check,reapply:reapplyPublicRuntime,widgetPanel:restoreWidgetPanel,stamp:stampVisibleVersion};
}
