...
function apply(key,cfg){const img=document.querySelector(`[data-home-media="${key}"]`);if(!img)return;const d=cfgFor(cfg,dev()),src=cfg.url||defaults[key].url;applyVisual(img,d,false);
  if(img.dataset.loadedSrc===src&&img.complete){img.style.opacity=String((Number(d.opacity)||100)/100);return}
  img.style.opacity='0';
  const pre=new Image();
  pre.onload=()=>{img.src=src;img.dataset.loadedSrc=src;requestAnimationFrame(()=>{applyVisual(img,d,true)})};
  pre.onerror=()=>{console.warn('RAF.studio: nie udało się wczytać obrazu',key,src);img.style.opacity='0'};
  pre.src=src;
}
