...
function render(raw={}){for(const key of Object.keys(defaults))apply(key,{...defaults[key],...(raw[key]||{})});}
onValue(ref(db,`${WEBSITE_ROOT}/public/homeMedia`),snap=>render(snap.val()||{}));
window.addEventListener('resize',()=>onValue(ref(db,`${WEBSITE_ROOT}/public/homeMedia`),snap=>render(snap.val()||{}),{onlyOnce:true}));
render();
