const params=new URLSearchParams(location.search),mode=params.get('editor'),LATEST='6.6.1',requested=params.get('ev')||LATEST;
...
async function waitEditorSettled(){
 ...
 while(performance.now()-started<3200){const toolbar=document.querySelector('#rafTop3,.peTop'),quiet=performance.now()-last>320;if(toolbar&&quiet&&document.readyState!=='loading')break;await new Promise(r=>setTimeout(r,70))}
 try{obs?.disconnect()}catch{}await new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r)));await new Promise(r=>setTimeout(r,90));releaseEditor()
}
if(editorMode&&requested!==LATEST){const u=new URL(location.href);u.searchParams.set('ev',LATEST);u.searchParams.set('_editorBuild','661');location.replace(u.toString())}
else if(editorMode){
 ...
 (async()=>{try{
  ...
  if(!portfolio){
   await import('./editor-recovery-v64.js?v=6.5.3');
   await import('./editor-baseline-sync-v652.js?v=6.5.3');
  }
  if(portfolio){
   await import('./portfolio-editor.js?v=6.2.0');await import('./portfolio-page-v4.js?v=4.0.0');await import('./portfolio-chrome-v47.js?v=6.5.3');
  }else{
   ...
   await import('./free-layout-migrate-v661.js?v=6.6.1');
   await import('./editor-free-transform-v66.js?v=6.6.1');
   await import('./editor-history-preview-v64.js?v=6.5.3');
   await import('./editor-chrome-v61.js?v=6.6.1');
  }
