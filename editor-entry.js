const params=new URLSearchParams(location.search),mode=params.get('editor'),LATEST='8.9.0',CURRENT_BUILD='8900',requested=params.get('ev')||LATEST;
...
(async()=>{if(editorMode&&requested!==LATEST&&explicitArchive){await bootArchived();return}await import('./hero-video-layout-v888.js?v=8.9.0');await import('./video-performance-v888.js?v=8.8.8');await import('./site-v800.js?v=8.9.0');if(!editorMode)await import('./accordion-runtime-v877.js?v=8.8.3');if(editorMode)await bootCurrent()})();
