...
// Adds explicit merge/split group controls, selection breadcrumbs and alignment
// inside an existing group without moving the surrounding section/container.
...
.v880groupActions{display:grid;grid-template-columns:1fr 1fr;gap:6px}.v880groupActions button,.v880align button{border:1px solid #ffffff20;background:#17181b;color:#fff;border-radius:8px;padding:8px 6px;cursor:pointer;font:800 9px system-ui}.v880groupActions button.primary{border-color:#a54cff88;background:#7d2cb0}.v880groupActions button:disabled,.v880align button:disabled{opacity:.32;cursor:not-allowed}
.v880align{display:grid;grid-template-columns:repeat(3,1fr);gap:5px;margin-top:6px}.v880align button{padding:7px 3px}.v880note{margin-top:7px;color:#737a82;font:9px/1.4 system-ui}
#v72g,#v72ug{font-weight:900!important}
`;
...
function sectionLabel(el){
 ...
 const raw=sec.dataset.rafSection||sec.dataset.homeSection||'';
 const map={Hero:'HERO',About:'O MNIE',TwoWorlds:'DWA ŚWIATY',Reviews:'OPINIE',Brands:'MARKI',Contact:'KONTAKT'};
 if(map[raw])return map[raw];
 if(raw)return String(raw).replace(/[_-]+/g,' ').toUpperCase();
...
function leafLabel(el){
 if(!el)return'ELEMENT';const i=detailsIndex(el);if(i){if(el.closest('summary'))return'PYTANIE '+i;if(el.closest('details>p'))return'ODPOWIEDŹ '+i}
 if(el.matches('img,picture'))return'ZDJĘCIE';if(el.matches('video'))return'FILM';if(el.matches('a,button,[role="button"]'))return'PRZYCISK';if(el.matches('h1,h2,h3,h4,h5,h6,blockquote'))return'NAGŁÓWEK';if(el.matches('p,span,b,strong,small,em,figcaption,label,li,summary'))return'TEKST';
 return (el.tagName||'ELEMENT').toUpperCase()
}
function crumbsFor(el){
 const out=[sectionLabel(el)],i=detailsIndex(el);if(i)out.push(el.closest('summary')?'Pytanie '+i:el.closest('details>p')?'Odpowiedź '+i:'FAQ '+i);out.push(leafLabel(el));return out
}
...
function wireLegacyButtons(items){
 const g=$('#v72g'),ug=$('#v72ug');if(g){g.textContent='🔗 SCAL ZAZNACZONE';g.disabled=items.length<2;g.onclick=mergeSelected}if(ug){ug.textContent='⛓ ROZDZIEL GRUPĘ';ug.disabled=!items.some(groupId);ug.onclick=splitGroup}
}
function render(){
 ...
 const g=sameGroup(items),crumbs=breadcrumb(items),box=document.createElement('div');box.id='v880groupTools';box.innerHTML=`<div id="v880crumb">${crumbs.map((x,i)=>`${i?'<span class="v880sep">›</span>':''}<span class="v880chip">${esc(x)}</span>`).join('')}</div><div class="v880head"><b>SMART GROUPS</b><small>${g?'grupa aktywna':'zaznaczenie'}</small></div><div class="v880groupActions"><button id="v880merge" class="primary" ${items.length<2?'disabled':''}>🔗 Scal zaznaczone</button><button id="v880split" ${items.some(groupId)?'':'disabled'}>⛓ Rozdziel grupę</button></div><div class="v880head" style="margin-top:11px"><b>WYRÓWNANIE WEWNĄTRZ GRUPY</b><small>${g?items.length+' el.':'najpierw scal'}</small></div><div class="v880align"><button data-v880-align="left" ${g?'':'disabled'}>← Lewo</button><button data-v880-align="center" ${g?'':'disabled'}>↔ Środek</button><button data-v880-align="right" ${g?'':'disabled'}>Prawo →</button><button data-v880-align="top" ${g?'':'disabled'}>↑ Góra</button><button data-v880-align="middle" ${g?'':'disabled'}>↕ Środek</button><button data-v880-align="bottom" ${g?'':'disabled'}>↓ Dół</button></div><div class="v880note">Wyrównanie rusza wyłącznie elementy należące do zaznaczonej grupy. Sekcja i pozostałe elementy strony zostają na miejscu.</div>`;
 const h3=panel.querySelector('h3');if(h3)h3.insertAdjacentElement('afterend',box);else panel.prepend(box);$('#v880merge',box).onclick=mergeSelected;$('#v880split',box).onclick=splitGroup;$$('[data-v880-align]',box).forEach(b=>b.onclick=()=>alignInside(b.dataset.v880Align))
}
...
let tries=0,t=setInterval(()=>{versionLabels();if(window.rafCore760||window.rafCore72){schedule();if(++tries>30)clearInterval(t)}},160);
