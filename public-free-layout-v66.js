// RAF.studio — public free layout v6.6.1 FALLBACK ONLY for v7
import { initializeApp,getApps,getApp } from "https://www.gstatic.com/firebasejs/12.2.1/firebase-app.js";
import { getDatabase,ref,onValue } from "https://www.gstatic.com/firebasejs/12.2.1/firebase-database.js";
import { firebaseConfig,WEBSITE_ROOT } from './firebase-config.js';
const Q=new URLSearchParams(location.search);if(!Q.has('editor')&&!Q.has('tplPreview')){
...
let layout={},timer=null,v7Active=false;
...
function apply(){if(v7Active)return;$$(candidates).forEach(el=>{const k=keyFor(el);if(!k)return;el.dataset.rafFreeKey=k;const c=cfg(k);if(!c||!Object.keys(c).length)return;el.style.position='relative';el.style.left=`${Number(c.x)||0}px`;el.style.top=`${Number(c.y)||0}px`;if(c.z)el.style.zIndex=String(c.z);if(c.hidden)el.style.display='none'})}
function schedule(){if(v7Active||window.RAF_RENDERER900_ACTIVE)return;clearTimeout(timer);requestAnimationFrame(apply);timer=setTimeout(apply,160);setTimeout(apply,650)}
onValue(ref(db,`${WEBSITE_ROOT}/public/builder/freeLayoutV7`),s=>{v7Active=s.exists()&&!!s.val();if(v7Active)clearTimeout(timer);else schedule()});
onValue(ref(db,`${WEBSITE_ROOT}/public/builder/freeLayout`),s=>{layout=s.val()||{};schedule()});
const obs=new MutationObserver(schedule);obs.observe(document.body,{childList:true});addEventListener('resize',schedule,{passive:true});
}
