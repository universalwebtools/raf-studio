...
(function(){
 ...
 function refreshVersionLabels(){
  const select=$('#editorVersion770 select');
  if(select){
   ...
   if((new URLSearchParams(location.search).get('ev')||'')==='8.7.7')select.value='8.7.7'
  }
  for(const el of $$('#v72panel small,#rafPanel3 small')){
   if(/V8\.7\.2 CORE/i.test(el.textContent||''))el.textContent=(el.textContent||'').replace(/V8\.7\.2 CORE/i,'V8.7.7 CORE')
  }
