// RAF.studio — layers, responsive controls, crop, scale and full typography UI v8.7.2
...
function css(){
 ...
 '#rafPanel3 #v760common{border:1px solid #22a8ff44;background:#07131b;margin:0 0 12px;padding:11px;border-radius:11px}#v760common .v760title{display:flex;justify-content:space-between;align-items:center;margin-bottom:7px}#v760common .v760title small{color:#70caff;font-weight:900;letter-spacing:.12em}#v760common .v760grid{display:grid;grid-template-columns:1fr 1fr;gap:6px}#v760common label{margin:5px 0 2px!important}#v760common input,#v760common select,#v760common textarea{box-sizing:border-box;padding:7px!important}#v760common textarea{min-height:86px;resize:vertical}#v760common .v760actions{display:grid;grid-template-columns:repeat(3,1fr);gap:5px;margin-top:8px}#v760common .v760actions button{padding:7px 4px!important;font-size:10px}#v760common .v850type{border-top:1px solid #43b9ff44;margin-top:10px;padding-top:10px}#v760common .v850typeTitle{font-size:10px;font-weight:900;letter-spacing:.13em;color:#70caff;margin-bottom:5px}#v760common .v850typeNote{font-size:9px;color:#8f9ba3;line-height:1.45;margin:6px 0}#v760common .v850typeActions{display:grid;grid-template-columns:1fr 1fr;gap:5px;margin-top:8px}#v760common .v850typeActions button{padding:7px 5px!important;font-size:9px}'+
...
function hexColor(value){if(/^#[0-9a-f]{6}$/i.test(value||''))return value;const m=String(value||'').match(/\d+/g);return m?'#'+m.slice(0,3).map(x=>(+x).toString(16).padStart(2,'0')).join(''):'#ffffff'}
function opt(value,label,current){return'<option value="'+value+'" '+(String(value)===String(current)?'selected':'')+'>'+label+'</option>'}
...
function inspectorHtml(el){
 const c=core.cfgFor(el),r=el.getBoundingClientRect(),w=Math.round(Number(c.width)||el.offsetWidth||r.width),h=Math.round(Number(c.height)||el.offsetHeight||r.height),scale=Math.round((Number(c.scale)||1)*100),isImg=el instanceof HTMLImageElement,isText=editableText(el),actionLink=el instanceof HTMLAnchorElement?el:el.closest?.('a[href]'),linkCfg=actionLink?core.cfgFor(actionLink):null,crop={x:50,y:50,zoom:1,fit:'cover',...(c.crop||{})},cs=getComputedStyle(el);
 const targets=[['#kontakt','Kontakt na tej stronie'],['/fotografia/','Fotografia'],['/film/','Film'],['/polityka-prywatnosci/','Polityka prywatności'],...(window.rafPages860?.targets?.()||[])];
 let html='<div class="v760title"><small>EDYCJA ELEMENTU • 9.0</small><span>'+esc(core.id(el))+'</span></div>'+(isText?'<label>Treść<textarea id="v760text">'+esc(c.text??el.textContent??'')+'</textarea></label>':'')+(actionLink?'<div class="v860link"><label>Link / akcja<input id="v760href" list="v860linkTargets" value="'+esc(linkCfg?.href??actionLink.getAttribute('href')??'')+'"><datalist id="v860linkTargets">'+targets.map(x=>'<option value="'+esc(x[0])+'">'+esc(x[1])+'</option>').join('')+'</datalist></label><label>Szybki cel<select id="v860linkSelect"><option value="">— wybierz —</option>'+targets.map(x=>'<option value="'+esc(x[0])+'">'+esc(x[1])+' · '+esc(x[0])+'</option>').join('')+'</select></label><div class="v850typeActions"><button id="v860pagesOpen">＋ Nowa podstrona</button><button id="v860testLink">↗ Testuj link</button></div><small class="v850typeNote">W edytorze kliknięcie przycisku nie otwiera strony. Użyj „Testuj link” albo Podglądu.</small></div>':'');
 if(isText){const family=String(c.fontFamily||cs.fontFamily||'Inter').replace(/["']/g,'').split(',')[0].trim(),fontSize=Number(c.fontSize)||parseFloat(cs.fontSize)||16,lineHeight=Number(c.lineHeight)||(parseFloat(cs.lineHeight)/Math.max(1,parseFloat(cs.fontSize)))||1.2,tracking=c.letterSpacing!=null?Number(c.letterSpacing):(parseFloat(cs.letterSpacing)||0),weight=String(c.fontWeight||cs.fontWeight||400),style=c.fontStyle||cs.fontStyle||'normal',align=c.textAlign||cs.textAlign||'left',transform=c.textTransform||cs.textTransform||'none',decoration=String(c.textDecoration||cs.textDecorationLine||'none').split(' ')[0],kerning=c.fontKerning||cs.fontKerning||'auto';html+='<div class="v850type"><div class="v850typeTitle">PEŁNA TYPOGRAFIA • '+(currentDevice()==='desktop'?'PC':currentDevice()==='tablet'?'TABLET':'TELEFON')+'</div><div class="v850typeNote">Każdy tekst ma teraz osobne ustawienia dla wybranego urządzenia.</div><label>Czcionka<input id="v850font" list="v850fonts" value="'+esc(family)+'"><datalist id="v850fonts"><option value="Inter"><option value="Arial"><option value="Helvetica"><option value="Segoe UI"><option value="Georgia"><option value="Times New Roman"><option value="Courier New"><option value="Impact"><option value="system-ui"></datalist></label><div class="v760grid"><label>Rozmiar px<input id="v850size" type="number" min="6" max="500" value="'+fontSize+'"></label><label>Grubość<select id="v850weight">'+[100,200,300,400,500,600,700,800,900].map(x=>opt(x,x,weight)).join('')+'</select></label><label>Interlinia<input id="v850line" type="number" min="0.5" max="4" step="0.05" value="'+Number(lineHeight).toFixed(2)+'"></label><label>Tracking px<input id="v850track" type="number" min="-30" max="100" step="0.25" value="'+tracking+'"></label><label>Styl<select id="v850style">'+opt('normal','Prosta',style)+opt('italic','Kursywa',style)+'</select></label><label>Kerning<select id="v850kern">'+opt('auto','Auto',kerning)+opt('normal','Normalny',kerning)+opt('none','Wyłączony',kerning)+'</select></label><label>Kolor<input id="v850color" type="color" value="'+hexColor(c.color||cs.color)+'"></label><label>Wyrównanie<select id="v850align">'+opt('left','Lewo',align)+opt('center','Środek',align)+opt('right','Prawo',align)+opt('justify','Wyjustuj',align)+'</select></label><label>Wielkość liter<select id="v850transform">'+opt('none','Bez zmiany',transform)+opt('uppercase','WIELKIE',transform)+opt('lowercase','małe',transform)+opt('capitalize','Pierwsze Wielkie',transform)+'</select></label><label>Dekoracja<select id="v850decor">'+opt('none','Brak',decoration)+opt('underline','Podkreślenie',decoration)+opt('line-through','Przekreślenie',decoration)+'</select></label></div><div class="v850typeActions"><button id="v850typeReset">↶ Styl szablonu</button>'+(currentDevice()!=='desktop'?'<button id="v850typeInherit">⇠ Dziedzicz z PC</button>':'')+'</div></div>'}
 html+='<div class="v760grid"><label>X<input id="v760x" type="number" value="'+Math.round(Number(c.x)||0)+'"></label><label>Y<input id="v760y" type="number" value="'+Math.round(Number(c.y)||0)+'"></label><label>Szerokość<input id="v760w" type="number" min="24" value="'+w+'"></label><label>Wysokość<input id="v760h" type="number" min="24" value="'+h+'"></label><label>Skala %<input id="v760scale" type="number" min="5" max="800" value="'+scale+'"></label><label>Obrót °<input id="v760rot" type="number" value="'+Math.round(Number(c.rotate)||0)+'"></label><label style="display:flex;align-items:center;gap:6px;padding-top:17px"><input id="v760ratio" type="checkbox" style="width:auto" checked> Proporcje</label></div><div class="v760actions"><button data-v760="duplicate">Duplikuj</button><button data-v760="front">Na górę</button><button data-v760="back">Na dół</button><button data-v760="hide">'+(c.hidden?'Pokaż':'Ukryj')+'</button><button data-v760="lock">'+(c.locked?'Odblokuj':'Zablokuj')+'</button><button data-v760="reset">Reset</button><button class="v760danger" style="grid-column:1/-1" data-v760="delete">⌫ USUŃ ZAZNACZENIE</button></div>';
...
function syncInspector(el){
 ...
 const vals={v760x:Math.round(Number(c.x)||0),v760y:Math.round(Number(c.y)||0),v760w:Math.round(Number(c.width)||el.offsetWidth||r.width),v760h:Math.round(Number(c.height)||el.offsetHeight||r.height),v760scale:Math.round((Number(c.scale)||1)*100),v760rot:Math.round(Number(c.rotate)||0)};
...
function renderInspector(force=false){
 if(!core||augmenting)return;if(window.rafCapabilityPanel900){$('#v760common')?.remove();return}const el=single(),p=$('#rafPanel3');if(!el||!p){$('#v760common')?.remove();return}
 ...
 try{
  ...
  const startAspect=(Number(core.cfgFor(el).width)||el.offsetWidth||el.getBoundingClientRect().width)/Math.max(1,Number(core.cfgFor(el).height)||el.offsetHeight||el.getBoundingClientRect().height);
  const patch=()=>{const out={x:Number($('#v760x').value)||0,y:Number($('#v760y').value)||0,width:Math.max(24,Number($('#v760w').value)||24),height:Math.max(24,Number($('#v760h').value)||24),scale:Math.max(.05,Math.min(8,(Number($('#v760scale').value)||100)/100)),rotate:Number($('#v760rot').value)||0};core.patchOne(el,out)};
  for(const id of ['v760x','v760y','v760scale','v760rot'])$('#'+id)?.addEventListener('change',patch);
  ...
  const text=$('#v760text',box),href=$('#v760href',box),actionLink=el instanceof HTMLAnchorElement?el:el.closest?.('a[href]');let contentPushed=false;const pushContent=()=>{if(!contentPushed){core.checkpoint();contentPushed=true}};if(text){text.addEventListener('focus',pushContent,{once:true});text.addEventListener('input',()=>{pushContent();core.patchOne(el,{text:text.value},{commitNow:false})})}if(href&&actionLink){const setHref=value=>{pushContent();href.value=value;core.patchOne(actionLink,{href:value},{commitNow:false})};href.addEventListener('focus',pushContent,{once:true});href.addEventListener('input',()=>setHref(href.value));$('#v860linkSelect',box)?.addEventListener('change',e=>{if(e.target.value)setHref(e.target.value)});$('#v860pagesOpen',box)?.addEventListener('click',()=>window.rafPages860?.open?.());$('#v860testLink',box)?.addEventListener('click',()=>{const value=String(href.value||'').trim();if(!value)return;window.open(new URL(value,location.origin).href,'_blank','noopener')})}
  if(editableText(el)){
   let typoPushed=false;const pushTypo=()=>{if(!typoPushed){core.checkpoint();typoPushed=true}};
   const typography=()=>{pushTypo();core.patchOne(el,{fontFamily:$('#v850font',box).value.trim()||null,fontSize:Number($('#v850size',box).value)||null,fontWeight:Number($('#v850weight',box).value)||null,fontStyle:$('#v850style',box).value,lineHeight:Number($('#v850line',box).value)||null,letterSpacing:Number($('#v850track',box).value)||0,color:$('#v850color',box).value,textAlign:$('#v850align',box).value,textTransform:$('#v850transform',box).value,textDecoration:$('#v850decor',box).value,fontKerning:$('#v850kern',box).value},{commitNow:false})};
   for(const id of ['v850font','v850size','v850weight','v850style','v850line','v850track','v850color','v850align','v850transform','v850decor','v850kern']){const input=$('#'+id,box);input?.addEventListener(id==='v850font'?'change':'input',typography);input?.addEventListener('focus',pushTypo,{once:true})}
   const clearTypography=()=>{pushTypo();core.clearProps?.(el,core.typographyKeys||[] ,{commitNow:false});setTimeout(()=>scheduleInspector(true),30)};
   $('#v850typeReset',box)?.addEventListener('click',clearTypography);$('#v850typeInherit',box)?.addEventListener('click',clearTypography)
  }
...
function installEvents(){
 ...
 window.addEventListener('raf:pages860-updated',()=>scheduleInspector(true));
