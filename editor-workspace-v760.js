...
function css(){
 ...
 '#rafPanel3 #v760common{border:1px solid #22a8ff44;background:#07131b;margin:0 0 12px;padding:11px;border-radius:11px}#v760common .v760title{display:flex;justify-content:space-between;align-items:center;margin-bottom:7px}#v760common .v760title small{color:#70caff;font-weight:900;letter-spacing:.12em}#v760common .v760grid{display:grid;grid-template-columns:1fr 1fr;gap:6px}#v760common label{margin:5px 0 2px!important}#v760common input,#v760common select{box-sizing:border-box;padding:7px!important}#v760common .v760actions{display:grid;grid-template-columns:repeat(3,1fr);gap:5px;margin-top:8px}#v760common .v760actions button{padding:7px 4px!important;font-size:10px}'+
...
function renderLayers(){
 ...
 const groups=new Map();for(const x of list){if(!groups.has(x.sectionId))groups.set(x.sectionId,[]);groups.get(x.sectionId).push(x)}
 let html='';
 for(const [sec,rows] of groups){const visible=rows.filter(x=>!q||(x.label+' '+x.id+' '+x.tag).toLowerCase().includes(q));if(!visible.length)continue;html+='<div class="v760section">'+esc(sectionName(sec))+'</div>';
  for(const x of visible){const c=x.cfg,indent=Math.min(4,depth(x.el))*11;html+='<div class="v760layer '+(chosen.has(x.id)?'active ':'')+(c.hidden?'hidden ':'')+(c.locked?'locked ':'')+'" draggable="true" data-layer-id="'+esc(x.id)+'"><div class="v760layerName" style="padding-left:'+(4+indent)+'px"><span class="v760layerTag">'+esc(x.tag)+'</span>'+esc(x.label||x.id)+'</div><button data-act="rename" title="Zmień nazwę">✎</button><button data-act="hide" title="Ukryj / pokaż">'+(c.hidden?'◌':'◉')+'</button><button data-act="lock" title="Zablokuj / odblokuj">'+(c.locked?'🔒':'🔓')+'</button></div>'}
 }
 box.innerHTML=html||'<div style="padding:18px;color:#777">Brak pasujących warstw.</div>';let dragging='';
 $$('.v760layer',box).forEach(row=>{
  const id=row.dataset.layerId;row.onclick=e=>{const act=e.target.closest('button')?.dataset.act;if(act){e.stopPropagation();const item=core.list().find(x=>x.id===id);if(!item)return;if(act==='hide')core.patchOne(item.el,{hidden:!item.cfg.hidden});if(act==='lock')core.patchOne(item.el,{locked:!item.cfg.locked});if(act==='rename'){const name=prompt('Nazwa warstwy:',item.cfg.label||item.label);if(name!==null){core.selectElement(item.el);core.rename(name)}}scheduleLayers();return}core.selectById(id);scheduleLayers()};
...
function contextMenu(){
 ...
 menu.innerHTML='<button data-c="copy">Kopiuj <small>Ctrl+C</small></button><button data-c="paste">Wklej <small>Ctrl+V</small></button><button data-c="duplicate">Duplikuj <small>Ctrl+D</small></button><hr><button data-c="copyStyle">Kopiuj wygląd</button><button data-c="pasteStyle">Wklej wygląd</button><hr><button data-c="front">Na samą górę</button><button data-c="back">Na sam dół</button><button data-c="hide">Ukryj / pokaż</button><button data-c="lock">Zablokuj / odblokuj</button><button data-c="reset">Reset pozycji i rozmiaru</button>';
