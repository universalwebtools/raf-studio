// RAF.studio — layers, responsive controls, crop and full typography UI v8.6.1
...
function inspectorHtml(el){
 const c=core.cfgFor(el),r=el.getBoundingClientRect(),w=Math.round(Number(c.width)||r.width),h=Math.round(Number(c.height)||r.height),isImg=el instanceof HTMLImageElement,isText=editableText(el),actionLink=el instanceof HTMLAnchorElement?el:el.closest?.('a[href]'),linkCfg=actionLink?core.cfgFor(actionLink):null,crop={x:50,y:50,zoom:1,fit:'cover',...(c.crop||{})},cs=getComputedStyle(el);
