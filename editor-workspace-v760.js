// RAF.studio — layers, responsive controls, crop and full typography UI v8.5.2
...
function inspectorHtml(el){
 const c=core.cfgFor(el),r=el.getBoundingClientRect(),w=Math.round(Number(c.width)||r.width),h=Math.round(Number(c.height)||r.height),isImg=el instanceof HTMLImageElement,isText=editableText(el),crop={x:50,y:50,zoom:1,fit:'cover',...(c.crop||{})},cs=getComputedStyle(el);
 let html='<div class="v760title"><small>EDYCJA ELEMENTU • 8.5</small><span>'+esc(core.id(el))+'</span></div>'+(isText?'<label>Treść<textarea id="v760text">'+esc(c.text??el.textContent??'')+'</textarea></label>':'')+(el instanceof HTMLAnchorElement?'<label>Link<input id="v760href" value="'+esc(c.href??el.getAttribute('href')??'')+'"></label>':'');
