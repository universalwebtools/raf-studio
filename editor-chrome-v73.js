// RAF.studio — editor chrome v8.8.4
const $=(s,r=document)=>r.querySelector(s);
const q=new URLSearchParams(location.search),LATEST=window.rafEditorReleases?.[0]||'8.8.4',ver=q.get('ev')||LATEST;
const LABELS={
 '8.8.4':'VIDEO ECO + PARITY + SMART GROUPS','8.8.3':'WIDGET VIDEO + PARITY + SMART GROUPS','8.8.2':'FAQ COMPONENTS + SMART GROUPS','8.8.1':'FAQ SCALONE + SMART GROUPS','8.8.0':'SMART GROUPS + BREADCRUMBS',
...
function versions(){const live=Array.isArray(window.rafEditorReleases)?window.rafEditorReleases:[LATEST,'8.8.3','8.8.2','8.8.1','8.8.0','8.7.7','8.7.6','8.7.2','8.7.1','8.7.0'];return [...new Set(live)]}
function init(){
 ...
 top.insertBefore(w,$('#templatesBtn752')||$('#proBtn61')||$('#add3')||top.firstChild);const s=w.querySelector('select');s.value=ver;s.onchange=()=>{if(window.rafGoEditorVersion){window.rafGoEditorVersion(s.value);return}const u=new URL(location.href);u.searchParams.set('editor','direct');u.searchParams.set('ev',s.value);u.searchParams.set('_editorBuild',window.rafEditorBuildFor?.(s.value)||'8840');location.href=u.toString()};
 const p=$('#pageSelect4');if(p)p.onchange=e=>{const u=new URL(e.target.value,location.href);u.searchParams.set('editor','direct');u.searchParams.set('ev',ver);u.searchParams.set('_editorBuild',window.rafEditorBuildFor?.(ver)||'8840');location.href=u.toString()};syncPanel();return true
}
